...
public class BourseMathsR {
	...
	public static void ajouter() {
		...
		for(int i=0;i<5;i++) {
			...
			if(i<1) {
				...
				modeltcm[i]=DataBase.importDonneBourseR(modeltcm[i], "MATHEMATIQUES ET APPLICATIONS", semestre[i],"T.C.M"); //ligne 468 data base
				tabletcm[i]=new JTable(modeltcm[i]);
				panelTabletcm[i]= new PanelBourseR(semestre[i]+" T.C.M",tabletcm[i],acttcm[i],modeltcm[i]);
				...
				modelmiss[i]=DataBase.importDonneBourseR(modelmiss[i], "MATHEMATIQUES ET APPLICATIONS", semestre[i],"M.I.S.S");
			    tablemiss[i]=new JTable(modelmiss[i]);
			    actmiss[i].setPreferredSize(new Dimension(100,25));
			    
				panelTablemiss[i]= new PanelBourseR(semestre[i]+" M.I.S.S ",tablemiss[i],actmiss[i],modelmiss[i]);
				...
				modelme[i]=DataBase.importDonneBourseR(modelme[i], "MATHEMATIQUES ET APPLICATIONS", semestre[i],"M.E");
			    tableme[i]=new JTable(modelme[i]);
			    actme[i].setPreferredSize(new Dimension(100,25));
				
				panelTableme[i]= new PanelBourseR(semestre[i]+" M.E ",tableme[i],actme[i],modelme[i]);
				...
				modelmf[i]=DataBase.importDonneBourseR(modelmf[i], "MATHEMATIQUES ET APPLICATIONS",semestre[i],"M.F");
			    tablemf[i]=new JTable(modelmf[i]);
			    actmf[i].setPreferredSize(new Dimension(100,25));
				
				panelTablemf[i]= new PanelBourseR(semestre[i]+" M.F ",tablemf[i],actmf[i],modelmf[i]);
	...
	public static class Actualiser implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			// TODO Auto-generated method stub
			model=DataBase.importDonneBourseR(model, "MATHEMATIQUES ET APPLICATIONS", sems, parc);
		}
